...
export default function ProbabilityInput({
    ...
    // گرادیانت: کم → سبز (چپ)   زیاد → قرمز (راست)
        ...
        >
          ...
          {label}
            ...
            "
                    ...
                    {/* لایه رنگی گرادیانت که با مقدار رشد می‌کند */}
                    <div
                        className={`
              absolute inset-y-0 right-0 h-1.5 sm:h-2 
              rounded-r-full overflow-hidden
              bg-gradient-to-l ${getGradient(value)}   /* به چپ = از زیاد به کم */
              transition-all duration-300
            `}
                        style={{ width: `${value}%` }}
                    />

                    {/* خود input range – فقط thumb قابل دیدن */}
                    <input
                        id={id}
                        type="range"
                        min={0}
